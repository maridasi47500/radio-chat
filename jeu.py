...
class Jeu(Model):
    def __init__(self):
        ...
        #self.con.close()
    def getall(self):
        self.cur.execute("select jeu.id, jeu.id as jeuid,song.id as songid,lyric.text,jeu.pic, song.title,artist.name as artistname from jeu left join lyric on lyric.id = jeu.lyric_id left join song on song.id = lyric.song_id left join artist on artist.id = song.artist_id",())
